...
var aslpDB = /** @class */ (function () {
	...
	aslpDB.prototype.findDocument = function (collection, query) {
		...
		return new Promise((resolve, reject) => {
			...
			this.db.collection(collection).findOne(query, { projection: { _id: 0 } }, function (error, docs) {
	...
	aslpDB.prototype.findDocuments = function (collection, query, limit = 100, sort = {}, skip = 0) {
		...
		return new Promise((resolve, reject) => {
			...
			this.db.collection(collection).find(query, { projection: { _id: 0 }, limit: limit, sort: sort, skip: skip }).toArray(function (error, docs) {
	...
	aslpDB.prototype.findDocumentsWithId = function (collection, query, limit = 100, sort = {}, skip = 0) {
		...
		return new Promise((resolve, reject) => {
			...
			this.db.collection(collection).find(query, { limit: limit, sort: sort, skip: skip }).toArray(function (error, docs) {
	...
	aslpDB.prototype.findDocumentCount = function (collection, query) {
		...
		return new Promise((resolve, reject) => {
			...
			this.db.collection(collection).find(query, {}).count(function (error, count) {
	...
	aslpDB.prototype.findDocumentBigSum = function (collection, query, sumfield) {
		...
		return new Promise((resolve, reject) => {
			...
			this.db.collection(collection).find(query, {}).toArray(function (error, results) {
	...
	aslpDB.prototype.findDocumentHash = function (collection, query, field, sort) {
		...
		return new Promise((resolve, reject) => {
			...
			this.db.collection(collection).find(query, { projection: { _id: 0 }, limit: 10, sort: sort, skip: 0 }).toArray(function (error, results) {
	...
	aslpDB.prototype.insertDocument = function (collection, query) {
		...
		return new Promise((resolve, reject) => {
			...
			this.db.collection(collection).insertOne(query, function (error, result) {
	...
	aslpDB.prototype.insertDocuments = function (collection, query) {
		...
		return new Promise((resolve, reject) => {
			...
			this.db.collection(collection).insertMany([query], function (error, result) {
	...
	aslpDB.prototype.createJournalEntry = function (txid, blockId, blockHeight, timestamp, timestampUnix, action, collectionName, fieldData, recordData) {
		...
		return new Promise((resolve, reject) => {
			...
			autoIncrement.getNextSequence(this.db, 'journal', "_id", function (err, autoIndex) {
				...
				this.db.collection('journal').insertOne(insertData, function (error, result) {
					...
					if (autoIndex == 1) // This is the first entry
					{
						...
						this.db.collection('journal').updateOne({ "_id": autoIndex }, { $set: { "chainHash": recordHash } }, function (error, updateresult) {
						...
						this.db.collection('journal').findOne({ '_id': lastIndex }, {}, function (error, docs) {
	...
	aslpDB.prototype.updateDocument = function (collection, findquery, setquery) {
		...
		return new Promise((resolve, reject) => {
			...
			this.db.collection(collection).updateOne(findquery, { $set: setquery }, function (error, result) {
	...
	aslpDB.prototype.removeDocuments = function (collection, query) {
		...
		return new Promise((resolve, reject) => {
			...
			this.db.collection(collection).deleteMany(query, function (error, result) {
	...
	aslpDB.prototype.createIndex = function (collection, query, unique = true) {
		...
		return new Promise((resolve, reject) => {
			...
			this.db.collection(collection).createIndex(query, { background: true, sparse: sparse, unique: unique }, function (error, result) {
	...
	aslpDB.prototype.createCollection = function (collection, options = {}) {
		...
		return new Promise((resolve, reject) => {
			...
			this.db.createCollection(collection, options, function (error, result) {
