...
var aslpDB = /** @class */ (function () {
	...
	aslpDB.prototype.connect = aync function () {
	...
	aslpDB.prototype.findDocument = async function (collection, query) {
		...
		try {
			var docs = await this.db.collection(collection).findOne(query, { projection: { _id: 0 } });
			return docs;
		} catch (e) {
			throw new Error(e);	
		}
	...
	aslpDB.prototype.findDocuments = async function (collection, query, limit = 100, sort = {}, skip = 0) {
		...
		try {
			var docs = await this.db.collection(collection).find(query, { projection: { _id: 0 }, limit: limit, sort: sort, skip: skip }).toArray();
			return docs;
		} catch (e) {
			throw new Error(e);;	
		}
	...
	aslpDB.prototype.findDocumentsWithId = async function (collection, query, limit = 100, sort = {}, skip = 0) {
		...
		try {
			var docs = await this.db.collection(collection).find(query, { limit: limit, sort: sort, skip: skip }).toArray();
			return docs;
		} catch (e) {
			throw new Error(e);	
		}
	...
	aslpDB.prototype.findDocumentCount = async function (collection, query) {
		...
		try {
			var count = await this.db.collection(collection).find(query, {}).count();
			return count;
		} catch (e) {
			throw new Error(e);
		}
	...
	aslpDB.prototype.findDocumentBigSum = async function (collection, query, sumfield) {
		...
		try {
			var results = await this.db.collection(collection).find(query, {}).toArray();
	...
	aslpDB.prototype.findDocumentHash = async function (collection, query, field, sort) {
		...
		try {
			...
			var results = await this.db.collection(collection).find(query, { projection: { _id: 0 }, limit: 10, sort: sort, skip: 0 }).toArray();
	...
	aslpDB.prototype.insertDocument = async function (collection, query) {
		...
		try {
			var result = await this.db.collection(collection).insertOne(query);
			return result;
		} catch (e) {
			throw new Error(e);
		}
	...
	aslpDB.prototype.insertDocuments = async function (collection, query) {
		...
		try {
			var result = this.db.collection(collection).insertMany([query]);
			return result;
		} catch (e) {
			throw new Error(e);
		}
	...
	aslpDB.prototype.createJournalEntry = async function (txid, blockId, blockHeight, timestamp, timestampUnix, action, collectionName, fieldData, recordData) {
		...
		try {
			...
			var autoIndex = await autoIncrement(db, 'journal', 'id');
			...
			var result = await this.db.collection('journal').insertOne(insertData);
			...
			if (autoIndex == 1) // This is the first entry
			{
				...
				var updateresult = await this.db.collection('journal').updateOne({ "_id": autoIndex }, { $set: { "chainHash": recordHash } });
				...
				var docs = await this.db.collection('journal').findOne({ '_id': lastIndex }, {});
				...
				var result = await collection.updateOne({ "_id": autoIndex }, { $set: { "chainHash": chainHash } });
	...
	aslpDB.prototype.updateDocument = async function (collection, findquery, setquery) {
		...
		try {
			var result = await this.db.collection(collection).updateOne(findquery, { $set: setquery });
			return result;
		} catch (e) {
			throw new Error(e);
		}
	...
	aslpDB.prototype.removeDocument = async function (collection, query) {
		...
		try {
			var result = await this.db.collection(collection).deleteOne(query);
			return result;
		} catch (e) {
			throw new Error(e);
		}
	...
	aslpDB.prototype.removeDocuments = async function (collection, query) {
		...
		try {
			var result = await this.db.collection(collection).deleteMany(query);
			return result;
		} catch (e) {
			throw new Error(e);
		}
	...
	aslpDB.prototype.createIndex = async function (collection, query, unique = true) {
		...
		try {
			...
			var result = await this.db.collection(collection).createIndex(query, { background: true, sparse: sparse, unique: unique });
			return result;
		} catch (e) {
			throw new Error(e);
		}
